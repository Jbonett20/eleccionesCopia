...
function crearVotante() {
    try {
        ...
        if ($usuario_rol == 3) {
            ...
            $id_lider = obtenerIdLiderDesdeSesion($usuario_id);
...
function editarVotante() {
    try {
        ...
        if ($usuario_rol == 3) {
            $id_lider_sesion = obtenerIdLiderDesdeSesion($usuario_id);
            if (empty($id_lider_sesion)) {
                echo json_encode(['success' => false, 'message' => 'Usuario líder no encontrado']);
                return;
            }
...
function eliminarVotante() {
    try {
        ...
        $usuario_rol = $_SESSION['usuario_rol'];
        if ($usuario_rol == 3) {
            $id_lider_sesion = obtenerIdLiderDesdeSesion($_SESSION['usuario_id']);
            if (empty($id_lider_sesion)) {
                echo json_encode(['success' => false, 'message' => 'Usuario líder no encontrado']);
                return;
            }
