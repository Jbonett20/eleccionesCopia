...
function importarVotantes() {
    try {
            ...
            if (!empty($linea_limpia)) {
                foreach ($linea as $celda) {
                    if ($celda !== null) {
                        ...
                        $celda_limpia = normalizarEncabezado($celda);
                        if ($celda_limpia === 'nombres') {
                            ...
                            foreach ($linea as $header) {
                                if ($header !== null) {
                                    ...
                                    $headers[] = normalizarEncabezado($header);
                                } else {
                                    $headers[] = '';
                                }
                            }
        ...
        for ($i = $linea_inicio; $i < count($datos_archivo); $i++) {
            ...
            $identificacion_original = trim($datos[$idx_identificacion] ?? '');
            $identificacion = normalizarIdentificacion($identificacion_original);
            $telefono = $idx_telefono !== false ? trim($datos[$idx_telefono] ?? '') : '';
            $mesa = $idx_mesa !== false ? trim($datos[$idx_mesa] ?? '') : '';
            $lugar_mesa = $idx_lugar_mesa !== false ? trim($datos[$idx_lugar_mesa] ?? '') : '';
            $identificacion_lider = $idx_identificacion_lider !== false ? normalizarIdentificacion($datos[$idx_identificacion_lider] ?? '') : '';
            ...
            // Validar que la identificación tenga solo dígitos
            if (!ctype_digit($identificacion)) {
                $errores[] = "Línea $linea_num: La identificación '$identificacion_original' no es válida (debe contener solo números) | Votante: $nombres $apellidos";
                $linea_num++;
                continue;
            }
            ...
            if ($validacion['existe']) {
                ...
                try {
                    ...
                    $duplicado_existente = DB::queryFirstRow(
                        "SELECT id_duplicado, nombre_usuario_intento, lugar_mesa FROM votantes_duplicados WHERE identificacion = ?",
                        $identificacion
                    );
                    ...
                    if ($duplicado_existente) {
                        ...
                        ], 'id_duplicado=?', $duplicado_existente['id_duplicado']);
                    } else {
...
function normalizarEncabezado($valor) {
    $texto = strtolower(trim(str_replace("\xEF\xBB\xBF", '', (string)$valor)));
